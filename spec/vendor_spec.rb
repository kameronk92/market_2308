require "./spec/spec_helper"

RSpec.describe "Vendor" do
  before(:each) do
    @item1 = Item.new({name: 'Peach', price: "$0.75"})
    @item2 = Item.new({name: 'Tomato', price: '$0.50'})
    @item3 = Item.new({name: "Peach-Raspberry Nice Cream", price: "$5.30"})
    @item4 = Item.new({name: "Banana Nice Cream", price: "$4.25"})
    @vendor1 = Vendor.new("Rocky Mountain Fresh")
    @vendor2 = Vendor.new("Ba-Nom-a-Nom")
    @vendor3 = Vendor.new("Palisade Peach Shack")
  end

  describe "#initialize" do
    it "creates an instance of vendor with vendor name as argument" do 
      expect(@vendor1).to be_an_instance_of(Vendor)
      expect(@vendor1.name).to be_an_instance_of(String)
    end

    it "comes with an inventory as empty hash" do
      expect(@vendor1.inventory).to be_an_instance_of(Hash)
      expect(@vendor1.inventory).to eq({})
    end
  end

  describe "#check_stock" do
    it "returns the vendor's inventory stock quantity for an item" do
      expect(@vendor1.check_stock(@item1)).to eq(0) 
    end
  end

  describe "#stock" do 
    it "adds an item and quantity to the inventory hash" do
      @vendor1.stock(@item1, 30)
      expect(@vendor1.inventory[@item1]).to eq(30)
      expect(@vendor1.inventory.keys).to eq([@item1])
      @vendor1.stock(@item1, 25)
      expect(@vendor1.inventory[@item1]).to eq(55)
      expect(@vendor1.inventory.keys).to eq([@item1])
    end

    it "can add a second item to the inventory hash" do
      @vendor1.stock(@item2, 12)
      expect(@vendor1.inventory[@item2]).to eq(12)
    end
  end

  describe "potential revenue" do 
    it "checks the potential revenue of a vendor" do
      @vendor1.stock(@item1, 35)
      @vendor1.stock(@item2, 7)
      @vendor2.stock(@item4, 50)
      @vendor2.stock(@item3, 25)
      @vendor3.stock(@item1, 65)
      expect(@vendor1.potential_revenue).to eq(29.75)
      expect(@vendor2.potential_revenue).to eq(345.00)
      expect(@vendor3.potential_revenue).to eq(48.75)
    end
  end
end
